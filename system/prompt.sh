...
bash_prompt_command() {
  ...
  PS1="${P_WHITE}{${P_YELLOW}$(date +"%y-%m-%d %H:%M:%S")${P_WHITE}} ${P_USER}${P_WHITE}@${P_HOST} ${P_YELLOW}${P_PWD} $(git_prompt_info) ${P_EXIT}\n${P_YELLOW}➤  ${P_RESET}"
}

## git-theme
# feel free to change git chars.
GIT_THEME_PROMPT_DIRTY=" ${P_RED}*"
GIT_THEME_PROMPT_CLEAN=""
GIT_THEME_PROMPT_PREFIX="${P_GREEN}("
GIT_THEME_PROMPT_SUFFIX="${P_GREEN})"
