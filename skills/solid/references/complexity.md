...
```pseudocode
// Over-engineered
component UserServiceFactoryProvider:
  static instance: UserServiceFactoryProvider
...
function processOrder(order, postProcessing):
  validate(order)
  calculateTax(order)
  save(order)
...
3. **No duplication** - DRY (but Rule of Three), single source of truth
